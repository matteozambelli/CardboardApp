...
Grazie alla versione Desktop di GitHub, la gestione del lavoro e l' interazione con il repository è resa ancora più semplificata. I comandi per l' aggiunta, la creazione e la clonazione di un repository direttamente accessibili dall'applicazione:
\newpage
\begin{figure}[h]
	\centering
...
Lo scopo di questa applicazione è il trattamento dell'ambliopia, non è necessario avere a disposizione un device costoso, al giorno d'oggi infatti, tutte le persone hanno uno smartphone e Google Cardboard è molto economico. Usando l'applicazione Car Racing Cardboard, le persone che soffrono di ambliopia possono migliorare il loro occhio pigro divertendosi.\\
Quest' idea di avere una penalizzazione proporzionata alle abilità del giocatore è stata applicata non solo alla quantità di informazioni fornite agli occhi, ma anche alla difficoltà vera e propria del gioco. Infatti, ad ogni ondata di nemici evitati, che equivale ad aver superato un livello, la velocità dei  nemici aumenta, in questo modo il gioco viene reso più dinamico, non resta identico fino al gameover, ed inoltre, in base alle prestazioni dell' utente, la difficoltà di gioco aumenta\cite{tesiTZ}.\\
...
Alla base di questo progetto (Car Racing Cardboard), si trovava l'idea di dare continuità al nostro lavoro di tesi triennale \virgolette{Progetto e sviluppo di un videogioco basato sulla penalizzazione per il trattamento dell'ambliopia}, nel quale gli aspetti fondamentali del lavoro era una penalizzazione dinamica delle informazioni visive fornite al giocatore per rendere il trattamento dell' ambliopia equilibrato con le capacità del giocatore, infatti più l'utente si dimostra capace, più viene penalizzato.
...
\end{itemize}
Questa certificazioni sul Google PLay Store, insieme al numero di download, contribuiscono a dare credibilità alla nostra applicazione.
\newpage
\section{Paper}
Riguardo l'applicazione Car Racing Cardboard è stato realizzato un paper dal titolo: \textit{A low-cost virtual reality game for amblyopia rehabilitation},ed è stato sottoposto alla commissione della conferenza rehab 2015 \footnote{http://www.rehab-workshop.org/}, ed è stato accettato:
\begin{lstlisting}
...
\end{lstlisting}
\paragraph{Di seguito riportato il contenuto del nostro elaborato, redatto in collaborazione con il Professor Angelo Gargantini e l' Ing. Silvia Bonfanti:}
\includepdf[pages={1,2,3,4}]{rehab2015_racecardboard.pdf} 
\section{Sviluppi futuri}
Questo progetto non è fine a se stesso, ci sono vari spunti per sviluppi futuri, di seguito eccone alcuni:
\begin{itemize}
	...
	In questo modo, sarà possibile rendere ancora più accattivante la ribilitazione, fornendo un ventaglio di soluzioni differenti, per catturare i gusti di tutti i bambini;
	...
	Grazie ad una collaborazione di questo tipo, sarebbe molto più facile per gli sviluppatori correggere eventuali errori, sia di implementazione, ma anche di logica, ad esempio una penalizzazione troppo repentina o troppo poco;
