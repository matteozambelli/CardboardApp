...
\begin{figure}
	\centering
	\includegraphics[width=0.45\linewidth, height=0.3\textheight]{immagini/android-logo.png}
...
\textbf{//TODO--- metterei qui la parte php relativa alla registrazione!!!}\\
\paragraph{email all'utente:} l'utente riceve una mail di conferma come questa (Figura \ref{conferma_registrazione})
...
\subsection*{Doctor call}//TODO
\subsection*{Report}//TODO qui metterei lo screen di fine gioco, adesso non riesco perchè il server è down e la foto della mail di riepilogo
\newpage
\section{Thread}
\newpage
\section{Asynctask}
...
\section{GitHub}Per il versioning del software e la sincronizzazione del lavoro è stato utilizzato github,%da modificare preno da wikipedia%
 GitHub è un servizio web di hosting per lo sviluppo di progetti software, che usa il sistema di controllo di versione Git. Può essere utilizzato anche per la condivisione e la modifica di file di testo e documenti revisionabili (sfruttando il sistema di controllo di versione dei file di Git).

GitHub offre diversi piani per repository privati sia a pagamento, sia gratuiti, molto utilizzati per lo sviluppo di progetti open-source. Nel maggio 2011, GitHub era il più popolare repository di progetti open source.

GitHub Inc. è stata fondata nel 2008 ed ha sede legale a San Francisco, California. Nel luglio 2012 la compagnia ha ricevuto 100 milioni di dollari da Andreessen Horowitz per avviare la compagnia.
%fine da modificare
...
\paragraph{Risultati promozione} sicuramente tale \virgolette{pubblicizzazione} ha infliuto nell numero di download dell'applicazione, possiamo notare un notevole picco dopo la data di rilascio: 
...
Il paper è stato sottoposto alla commissione della conferenza rehab 2015\footnote{http://rehab.greenlines-institute.org/}, ed è stato accettato:
\begin{lstlisting}
